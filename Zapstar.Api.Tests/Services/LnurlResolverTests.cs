...
public class LnurlResolverTests
{
    ...
    [Fact]
    public async Task IsValidLightningAddressAsync_ReturnsTrue_ForRealLnurlPayResponse()
    {
        var handler = new FakeHttpMessageHandler().When(
            "/.well-known/lnurlp/tobses",
            HttpStatusCode.OK,
    ...
    [Fact]
    public async Task IsValidLightningAddressAsync_ReturnsFalse_ForPlainEmailWith404()
    {
        // Plain emails have no well-known LNURL endpoint - the handler falls through to its default 404.
    ...
    [Fact]
    public async Task IsValidLightningAddressAsync_ReturnsFalse_WhenTagIsNotPayRequest()
    {
        var handler = new FakeHttpMessageHandler().When(
            "/.well-known/lnurlp/someone",
            HttpStatusCode.OK,
    ...
    [Fact]
    public async Task IsValidLightningAddressAsync_ReturnsFalse_ForMalformedAddress()
    {
        var handler = new FakeHttpMessageHandler();
        var resolver = BuildResolver(handler);
    ...
    [Fact]
    public async Task GetInvoiceAsync_ReturnsError_WhenAmountBelowMinSendable()
    {
        var handler = new FakeHttpMessageHandler().When(
            "/.well-known/lnurlp/tobses",
            HttpStatusCode.OK,
        ...
        var resolver = BuildResolver(handler);
        // amountSats=1 -> 1000 msats, below minSendable of 5000 msats
    ...
    [Fact]
    public async Task GetInvoiceAsync_ReturnsInvoice_OnSuccess()
    {
        var handler = new FakeHttpMessageHandler()
            .When("/.well-known/lnurlp/tobses", HttpStatusCode.OK,
                """{"callback":"https://btcpay.example.com/pay/tobses","tag":"payRequest","minSendable":1000,"maxSendable":100000000}""")
            .When("/pay/tobses", HttpStatusCode.OK,
                """{"pr":"lnbc1u1p...fakeinvoice"}""");
    ...
    [Fact]
    public async Task GetInvoiceAsync_ReturnsError_WhenCallbackReturnsErrorStatus()
    {
        var handler = new FakeHttpMessageHandler()
            .When("/.well-known/lnurlp/tobses", HttpStatusCode.OK,
                """{"callback":"https://btcpay.example.com/pay/tobses","tag":"payRequest","minSendable":1000,"maxSendable":100000000}""")
            .When("/pay/tobses", HttpStatusCode.OK,
                """{"status":"ERROR","reason":"Invoice generation failed"}""");
